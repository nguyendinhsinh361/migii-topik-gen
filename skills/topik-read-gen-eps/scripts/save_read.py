#!/usr/bin/env python3
"""
save_read.py — Lưu câu hỏi đọc EPS-TOPIK đã gen ra CSV theo từng kind.

Nằm trong: skills/topik-read-gen-eps/scripts/

Mỗi câu hỏi = 1 dòng CSV. Content items được đánh số _1, _2, ... theo count_question.

Cách dùng:
  python skills/topik-read-gen-eps/scripts/save_read.py input.json
  python skills/topik-read-gen-eps/scripts/save_read.py input.json -o output/read-eps
  python skills/topik-read-gen-eps/scripts/save_read.py input.json --append
  python skills/topik-read-gen-eps/scripts/save_read.py input.json --validate-only
...
# scripts/ → topik-read-gen-eps/ → skills/ → project root
SKILL_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
PROJECT_ROOT = os.path.dirname(os.path.dirname(SKILL_DIR))
DEFAULT_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output", "read-eps")
...
def flatten_question(question, timestamp=None, seq=0):
    ...
    for idx, content in enumerate(content_list):
        ...
        # Image description — lấy trường "image" từ q_image_description
        img_desc = content.get("q_image_description", {})
        img_text = img_desc.get("image", "") if isinstance(img_desc, dict) else ""
...
def validate_question(question):
    """
    Validate 1 câu hỏi đọc EPS-TOPIK, trả về list các lỗi (rỗng = OK).

    Kiểm tra:
      1. Các trường bắt buộc tồn tại
      2. q_correct nằm trong 1-4
      3. 4 đáp án không trùng nhau
      4. count_question khớp len(content) (nếu không null)
      5. level hợp lệ (EPS-TOPIK chỉ có level 3)
      6. explain có cả vi và en
      7. tag = "read"
      8. Đoạn đọc không rỗng (q_text hoặc g_text hoặc q_image)

    Lưu ý EPS: q_point và count_question có thể null.
    """
    errors = []
    kind = str(question.get("kind", ""))
    level = question.get("level")
    ...
    # 7. tag phải là "read"
    if question.get("tag") != "read":
        errors.append(f"tag='{question.get('tag')}' phải là 'read'")

    # 8. Bài đọc — ít nhất 1 trong: g_text, q_text, q_image phải có nội dung
    ...
    # 4. count_question — EPS chấp nhận null, chỉ cảnh báo khi khai sai
    ...
    # 5. level hợp lệ — EPS chỉ có level 3
    valid_levels = {3}
    if level not in valid_levels:
        errors.append(f"level={level} không hợp lệ (EPS-TOPIK phải là 3)")

    # Kiểm tra từng content item
    for idx, content in enumerate(content_list):
        ...
        # q_point — EPS cho phép null, không báo lỗi
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Luu cau hoi doc EPS-TOPIK tu JSON ra CSV theo kind"
    )
